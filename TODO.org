...
* TODO Communicating Machines
CSP×UTM
** Machine structure
machines are essentially a bunch of threads and channels, possibly some state
** crash-only design
it should always be possible to crash and restart a machine.
*** initial state
a machine's definition should include instructions on how to "reset" a machine
to its starting state
*** fail state
the definition should include a way for a watchdog to detect whether a machine
has failed. it should be possible to perform this check externally; ie, it
should be possible for a watchdog to detect when a machine has entered a loop,
or is stuck in a system call, or whatnot. when the watchdog detects such a
situation, it simply puts the machine into its initial state.
* TODO Basis Tracking
